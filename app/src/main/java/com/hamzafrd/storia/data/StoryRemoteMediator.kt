package com.hamzafrd.storia.data
...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val responseData = entityToArray(page, state.config.pageSize)
            val endOfPaginationReached = responseData.isEmpty()
            database.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    database.remoteKeysDao().deleteRemoteKeys()

                    database.storyDao().deleteAllPagedStories()
                }
                ...
                database.storyDao().insertPagedStory(responseData)
            }
    ...
    private suspend fun entityToArray(page: Int, pageSize: Int): ArrayList<StoryEntity> {
        val data = ArrayList<StoryEntity>()
        apiService.getPagingStories(page, pageSize).listStory.forEach { story ->
            data.add(
                StoryEntity(
                    story.id,
                    story.createdAt,
